...
/*$Id: GraphViz.java 630 2016-03-08 17:07:11Z niko-rodrigue $*/
/*
 ******************************************************************************
 *                                                                            *
 ...
 * </dl>
 *
 * @version v0.2 2009-07-07 (EBI release)
...
public class GraphViz
{
  ...
  private final static String DOT_PATH_PROPERTY_NAME="sbml2dot.dot.path";
    ...
    * @return A byte array containing the image of the graph in the png format..
    */
   public byte[] getGraph(String dot_source) 
   {
	   return getGraph(dot_source, "");
   }
   ...
   public byte[] getGraph(String dot_source, String dotOptions)
   {
      ...
      try {
         ...
         if (dot != null)
         {
            img_stream = getImgStream(dot, dotOptions);
            if (dot.delete() == false) 
   ...
   private byte[] getImgStream(File dot, String dotAttr)
   {
      ...
      try {
         ...
         String cmd = DOT + " -T" + IMAGE_FORMAT + " " + dotAttr + " " + dot.getAbsolutePath() + " -o" + imagePath;
